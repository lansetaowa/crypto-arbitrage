...
import threading
from monitor_exit import monitor_exit_loop
...
# 记录在套利操作中的交易对与下单信息
# gate空 + binance多，格式：{symbol: {'bi_qty': x, 'gate_size': xx, 'funding_time': datetime, 'bi_entry_price': xx, 'gate_entry_price': xx, 'trade_type': 'type1'}}
active_type1 = {}
# gate多 + binance空，格式：{symbol: {'bi_qty': x, 'gate_size': xx, 'funding_time': datetime, 'bi_entry_price': xx, 'gate_entry_price': xx, 'trade_type': 'type2'}}
active_type2 = {}
...
def run_arbitrage():
    ...
    # 启动监控平仓线程
    threading.Thread(target=monitor_exit_loop,
                     args=(bdata_handler, gdata_handler, bf_trader, gf_trader, active_type1, active_type_lock),
                     daemon=True).start()
    threading.Thread(target=monitor_exit_loop,
                     args=(bdata_handler, gdata_handler, bf_trader, gf_trader, active_type2, active_type_lock),
                     daemon=True).start()
